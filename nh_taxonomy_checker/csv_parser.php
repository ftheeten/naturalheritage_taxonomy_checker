...
require_once("ParseFishbaseJSON.php");
...
class CSVParser 
{
            ...
            try
            {
                    ...
                    if(strlen(trim($sc_name))>0)
                    {
                        ...
                        //{ 
                            ...
                            if(in_array("fishbase",$this->sources))
                            {
                            
                                $results_fishbase=Array();
                                ////$this->debug("INIT_".$sc_name."!\n");
								 ////$this->debug("ORI_".$original_name."!\n");
								if($canonical_set===false)
								{   
                                
								//$this->debug("DO Canonical!\n");
									$parserName = new GNNameParser($original_name);
									$sc_name=$parserName->getCanonicalName();
									$canonical_set=true;
                                    
								}
								//$this->debug("CANO_".$sc_name."!\n");
                                $fishbase_client=new ParseFishbaseJSON($sc_name, $original_name);
                                $results_fishbase= $fishbase_client->returnResult();
                                $result=array_merge($result, $results_fishbase);
                            }
		...
		public function parseFile()
		{
			...
			if($handler = fopen($this->srcFile, "r"))
			{
                ...
                $sess_id=session_id();
                $_SESSION[$sess_id]["headers"] = $this->headers; 
                $_SESSION[$sess_id]["results"] = $this->completeResult; //$this->holder; 
