...
$EXCEPTION_FILE="/var/www/html/nh_taxonomy_checker/debug/exception.log";
function handleParsedCSV($p_page)
{
   
    $pager= new HTMLPager($_SESSION[session_id()]["results"], "50", "./parse_file.php?id=".session_id()."&page=", $_SESSION[session_id()]["headers"],$_SESSION[session_id()]["sources"]);
    print("PAGE = ". $p_page." ");
...
function handleCSV($file, $mail, $field, $kingdomfield)
{
    ...
    if(array_key_exists("source_fishbase", $_REQUEST))
    {
        $sources[]="fishbase";
    }
    ...
    try
   {   
        $_SESSION[session_id()]["sources"]=$sources;
        ...
        $urlTmp="https://darwin.naturalsciences.be/".preg_replace("/\/([^\/]+)$/","/async_parser.php?id=".session_id(),$_SERVER["REQUEST_URI"]);
